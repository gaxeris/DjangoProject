...
class PostViewSetTest(TestCase):
    def setUp(self) -> None:
        self.user = User.objects.create(username="author", password="author")
        self.user.save()

        self.category = Category.objects.create(
            name="Testing", description="Its a category to test out some features"
        )
        self.category.save()
    ...
    def test_post_single_post(self):
        ...
        client.force_authenticate(user=self.user)
        response = client.post(
            reverse("blog:post-list"), data=valid_post_data, format="json"
        )
